...
void BenchmarkMalloc(size_t ntimes, size_t nworks, size_t rounds)
{
	...
	printf("%ld个线程并发执行%ld轮次，每轮次malloc %ld次: 花费：%ld ms\n",
		nworks, rounds, ntimes, malloc_costtime.load());

	printf("%ld个线程并发执行%ld轮次，每轮次free %ld次: 花费：%ld ms\n",
		nworks, rounds, ntimes, free_costtime.load());

	printf("%ld个线程并发malloc&free %ld次，总计花费：%ld ms\n",
...
void BenchmarkConcurrentMalloc(size_t ntimes, size_t nworks, size_t rounds)
{
	...
	printf("%ld个线程并发执行%ld轮次，每轮次concurrent alloc %ld次: 花费：%ld ms\n",
		nworks, rounds, ntimes, malloc_costtime.load());

	printf("%ld个线程并发执行%ld轮次，每轮次concurrent dealloc %ld次: 花费：%ld ms\n",
		nworks, rounds, ntimes, free_costtime.load());

	printf("%ld个线程并发concurrent alloc&dealloc %ld次，总计花费：%ld ms\n",
		nworks, nworks * rounds * ntimes, malloc_costtime.load() + free_costtime.load());
}
